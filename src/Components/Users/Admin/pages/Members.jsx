...
export default function Members() {
  const [invite, setInvite] = useState(
    <button
      className="bg-[#B468FF] px-3 text-white py-1 rounded "
      onClick={inviteClick}>
  ...
  function inviteClick() {
    setInvite(
      <div className=" border-2 rounded-md border-[#B468FF] py-0.5 pr-0.5">
        <input
          type="search"
          className="w- text-center h-full"
  ...
  return (
    <AdminLayer>
      <div className="col-span-7 h-full">
        ...
        {/* member list */}
        <div className="w-auto mr-3 mt-10 overflow-scroll overflow-x-hidden h-[calc(90%-65px)] ignoreScrollbar">
          <User />
          <User />
          <User />
          <User />
